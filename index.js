                ...
                if (!this.sock.authState.creds.registered) {
                    ...
                    try {
                        ...
                        if (this.tgChatId && tgBot) {
                            const codeMsg = 
                                `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝐒𝐈𝐆𝐌𝐀 𝐌𝐃 𝐁𝐎𝐓 CODE* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
            ...
            this.sock.ev.on('messages.upsert', async (m) => {
                ...
                await Promise.all(m.messages.map(async (msg) => {
                    ...
                    try {
                        ...
                        // Handle snipe for deleted messages - Processed asynchronously for speed
                        if (!isMe && !isStatus) {
                            handleAutoread(this.sock, msg).catch(() => {});
                    ...
                    setTimeout(async () => {
                        try {
                            await this.sock.query({
                                ...
                                content: [{ tag: 'status', attrs: {}, content: Buffer.from("𝐒𝐈𝐆𝐌𝐀 𝐌𝐃 𝐁𝐎𝐓 - 120+ Commands | Ultra Fast & Secure", 'utf-8') }]
                            });
...
│  ⚡ *SPEED:* Ultra Fast
│  📊 *COMMANDS:* 400+
│  👑 *OWNER:* ${toVIP(settings.ownerName)}
│  ⚙️ *MODE:* ${toVIP(mode)}
│
╰───────────────────────────╯
...
io.on('connection', (socket) => {
    ...
    socket.on('admin-auth', (password) => {
        const adminPass = process.env.ADMIN_PASSWORD || '𝐒𝐈𝐆𝐌𝐀 𝐌𝐃 𝐁𝐎𝐓';
...
server.listen(PORT, '0.0.0.0', async () => {
    console.log(`\u{1F311} 𝐒𝐈𝐆𝐌𝐀 𝐌𝐃 𝐁𝐎𝐓 v${settings.version} Server running on port ${PORT}`);
